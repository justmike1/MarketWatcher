...
price_fetcher_dict = {
    ...
    "2500/Asset": lambda res_data: round(
        float(2500 / (next(iter(res_data.values()))["usd"])), 4
    ),
...
class MarketWatcher:
    def __init__(self):
        ...
        self.assets_markup = ReplyKeyboardMarkup(
            self.reply_keyboard, one_time_keyboard=False
        )
                    ...
                    for asset in self.assets:
                        if asset["ticker"] == entry["ticker"]:
                            ...
                            if datetime.now() - entry["timestamp"] >= timedelta(
                                seconds=self.price_change_interval
                            ):
                                ...
                                price_change = (new_price - entry["price"]) / entry[
                                    "price"
                                ]
    ...
    def start(self, update: Update, context):
        reply_text = (
            f'*Hi {update.message.from_user["first_name"]}\! Welcome to MarketWatcher bot\.*\n'
                    ...
                    for fetch_dict in asset["fetch_url"]:
                        for exchange, url in fetch_dict.items():
                            ...
                            if price > 0:
                                ...
                                if not updated:
                                    self.update_asset_price(
                                        update.effective_chat.id, asset["ticker"], price
                                    )
                                    updated = True
                            else:
                                logging.error(
    ...
    def asset_info(self, update: Update, context):
        ...
        context.bot.send_message(
            chat_id=update.effective_chat.id,
            text=f"{requested_ticker} Not found in asset list.",
            reply_markup=self.assets_markup,
        )
        logging.warning(
    ...
    def main(self):
        ...
        dispatcher.add_handler(
            MessageHandler(Filters.text & (~Filters.command), self.asset_info)
        )
